...
from src.lib.maimaidx.tool import hash
from src.lib.maimaidx.maimaidx_music import *
from src.lib.maimaidx.image import *
from src.lib.maimaidx.maimai_best_40 import generate
from src.lib.maimaidx.maimai_best_50 import generate50
import re
...
async def _(bot: Bot, event: Event, state: T_State):
    ...
    if len(res) == 0:
        ...
        await search_music.finish(Message(MessageSegment(
            type="text",
            data={
...
谱师: {chart['charter']}'''
        ...
        try:
            file = f"https://www.diving-fish.com/covers/{music['id']}.jpg"
            await query_chart.send(Message(MessageSegment(
                    type="text",
                    data={
                        "text": f"{music['id']}. {music['title']}\n"
                    }
...
async def _(bot: Bot, event: Event, state: T_State):
    ...
    for i in range(11):
        if wm_value[i] == 3:
            ...
            s += f'忌 {wm_list[i]}\n'
    s += "DuNaiBot提醒您：打机时不要大力拍打或滑动哦\n今日推荐歌曲："
    music = total_list[h % len(total_list)]
    await jrwm.finish(Message(MessageSegment(
...
async def _(bot: Bot, event: Event, state: T_State):
    ...
    if len(result_set) == 1:
        ...
        await find_song.finish(Message(MessageSegment(type="text", data={"text": "您要找的是不是"}) + song_txt(music)))
    else:
        s = '\n'.join(result_set)
...
BREAK\t5/12.5/25(外加200落)'''
        await query_score.send(Message(MessageSegment(
            type="image",
            data= {
...
best_40_pic = on_command('maib40')


@best_40_pic.handle()
async def _(bot: Bot, event: Event, state: T_State):
    username = str(event.get_message()).strip("/maib40 ")
    print(username)
    if username == "":
    ...
    if success == 400:
        ...
        await best_40_pic.send(Message(MessageSegment(
...
best_50_pic = on_command('maib50')


@best_50_pic.handle()
async def _(bot: Bot, event: Event, state: T_State):
    username = str(event.get_message()).strip("/maib50 ")
    if username == "":
    ...
    if success == 400:
        ...
        await best_50_pic.send(Message(MessageSegment(
